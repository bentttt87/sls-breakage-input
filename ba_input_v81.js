// SLS Breakage Input v82 — direct BA print, BA metadata, same-BA/SJ item flow.
(function(){
  'use strict';
  const get=id=>document.getElementById(id);
  let LAST_BA=null,REUSE_BA_NO=null;
  ...
  function setBuild(){const b=get('buildBadge');if(b)b.textContent='v82';}
  ...
  rpc=async function(fn,params={}){
    let snap=null;
    if((fn==='breakage_incident_create_v45'||fn==='breakage_incident_update_draft_v45')&&params?.p_payload){snap=snapshotForm();params={...params,p_payload:{...params.p_payload,ba_receiver_name:up(get('fReceiver')?.value||snap.receiver),ba_witness_name:up(get('fWitness')?.value||snap.witness),factory:up(get('fFactory')?.value||snap.factory)}};if(fn==='breakage_incident_create_v45'&&REUSE_BA_NO)params.p_payload.reuse_no_ba=REUSE_BA_NO;}
    ...
    if(fn==='breakage_incident_create_v45'&&snap&&String(params?.p_payload?.incident_type||'').toLowerCase()==='delivery'){REUSE_BA_NO=res?.no_ba||REUSE_BA_NO;LAST_BA={...snap,incident_id:res?.incident_id,incident_no:res?.incident_no,no_ba:res?.no_ba,status:res?.status||'DRAFT'};setTimeout(()=>showBaSuccess(LAST_BA),1050);}
    return res;
  };

  function groupMatch(r,s){if(String(r.incident_type||'').toLowerCase()!=='delivery')return false;if(s.no_ba&&r.no_ba)return same(r.no_ba,s.no_ba);return same(r.rdc,s.rdc)&&same(r.occurrence_date,s.date)&&same(r.no_sj,s.sj)&&same(r.customer,s.customer)&&same(r.transporter,s.transporter)&&same(r.vehicle_no,s.police);}
  ...
  document.addEventListener('click',e=>{if(e.target?.id==='newBtn'||e.target?.id==='navInput')clearReuse()},true);
  const baseEditIncident=editIncident;
  editIncident=async function(id){clearReuse();const row=incidentRows().find(x=>Number(x.incident_id)===Number(id));const r=await baseEditIncident.apply(this,arguments);setTimeout(()=>{ensureBaFields();if(get('fReceiver'))get('fReceiver').value=row?.ba_receiver_name||'';if(get('fWitness'))get('fWitness').value=row?.ba_witness_name||'';},120);return r;};window.editIncident=editIncident;
