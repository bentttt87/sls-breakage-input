// SLS Breakage Input v75 — UI polish for Database Ekspedisi.
(function(){
  'use strict';
  function apply(){
    // Keep only one Database Ekspedisi button on the SPV home action row.
    ...
    // Inject local styles once.
    if(!document.getElementById('logisticsPolishV75')){
      const st=document.createElement('style');
      st.id='logisticsPolishV75';
      st.textContent=`
        #logisticsModal .modal{width:min(760px,96vw)}
        #logisticsModal .modal-body{padding:14px 16px 20px}
        #logisticsModal .field{margin-bottom:0}
        ...
        #logisticsModal #lgVehicleForm>div[style*="display:flex"]{
          justify-content:flex-end!important;align-items:center;gap:8px!important;flex-wrap:wrap
        }
        ...
        #logisticsModal #lgVehicleSave{
          min-height:40px;padding:9px 14px;border-radius:9px;white-space:nowrap
        }
        #logisticsModal #lgVendorCancel,
        #logisticsModal #lgDriverCancel,
    ...
    // Clean button labels and order consistently: Batal | Simpan.
